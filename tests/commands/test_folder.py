...
def test_folder_cli_runs():
    # Mock analyze_file so we don't run the full analysis
    with patch("analysis.cli.commands.folder.analyze_folder") as mock_analyze, \
         patch("analysis.cli.commands.folder.plt") as mock_plt, \
         patch("sys.argv", [
             "analysis",                # program name
             "folder",                 # subcommand
             "folder1",
             "--model", "Fe55Forest", "Gaussian",
             "--sigmaleft", "2.0",
             "--sigmaright", "3.0",
             "--W", "30",
             "--capacity", "2e-12",
        ...
        assert args[1] == [aptapy.models.Fe55Forest, aptapy.models.Gaussian]
        assert args[2] == 30              # W
