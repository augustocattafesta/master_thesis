...
def test_compare_cli_runs():
    # Mock analyze_file so we don't run the full analysis
    with patch("analysis.cli.commands.compare.compare_folders") as mock_analyze, \
         patch("analysis.cli.commands.compare.plt") as mock_plt, \
         patch("sys.argv", [
             "analysis",                # program name
             "compare",                 # subcommand
             "folder1", "folder2",      # folder names
             "--model", "Gaussian",
             "--sigmaleft", "2.0",
             "--sigmaright", "3.0",
             "--W", "30",
