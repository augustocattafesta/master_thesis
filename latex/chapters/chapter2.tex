...
Since its birth in the 1960s \cite{giacconi2003}, X-ray astronomy has represented a powerful tool to study high-energy phenomena that involve celestial objects across the universe. The possibility to observe photons produced by galactic and extragalactic sources, such as neutron stars, binary systems, supernova remnants or active galactic nuclei (AGN), is made possible by the small probability of interaction with the low-density interstellar medium (ISM) and intergalactic medium (IGM) for energies above 0.1-1 keV. The main problem of X-ray astronomy arises when photons reach Earth's atmosphere, which is much denser than ISM and IGM, and they are easily absorbed and can not reach the surface. For comparison, atmosphere's outer layers have a density of about $10^{17}$ atoms cm$^{-3}$, while for ISM the average on the galactic plane is 1 atom cm$^{-3}$ and for IGM is $10^{-6}$--$10^{-7}$ atoms cm$^{-3}$ \cite{longair2011}, which correspond to a mean free path of about 10 m, 3 kpc and 3 Gpc for a 1 keV photon, respectively. The presence of the atmosphere makes it necessary to build space telescopes on board of satellites and send them into orbit around the Earth or the Sun. This is the reason why X-ray astronomy was born only after the progress of rocket science, when it became possible to launch the first detectors on sub-orbital flights, with which the first extrasolar sources were discovered, and then on satellites, which allowed to improve observations thanks to the increased available observation time and to fully dedicates missions. Fig. \ref{fig:transparency} shows how the altitude at which atmosphere becomes transparent varies with photon energy.
...
\begin{figure}[!t]
    ...
    \caption{High resolution spectrum and image of supernova remnant Cas A, obtained with Chandra X-ray observatory. The spectrum shows the main emission lines up to Fe K$\alpha$ and the continuum is produced by thermal Bremsstrahlung because of the high temperatures reached by the ejecta. The image shows different shells, each with a different color and representing a different element emission, including silicon, sulfur, calcium and iron. Images courtesy of NASA/CXC/SAO}
    \label{fig:cas_a}
\end{figure}

Studying temporal variations in the arrival time of photons allows to understand and monitor time dependent phenomena, which can occur on timescales that range from years to milliseconds. In particular, short timescales are more difficult to study due to instrumental difficulties, but they can give important information about compact objects, such as pulsars and black holes in binary systems. By tracking variations of rotational period, it is possible to understand how accreting mechanisms work. High resolution timing enables the validation of fundamental theories such as general relativity and even place constraints on equations of state of neutron stars. 

Polarization measurements reveal details about the source geometry and its surrounding environment. In particular, these measurements enables probe how magnetic fields are distributed, how they interact with matter and the imapact on emission processes \cite{xray_polarimetry}. Another important aspect is the possibility to measure how X-rays interact with matter around the source, because scattering processes can polarize light. An interesting example is the study of magnetic fields around supernova remnants, which are responsible for galactic cosmic rays acceleration, and pulsars, where they are at the basis of the emission of relativistic jets.
...
\begin{figure}[htbp]
    ...
    \caption{Number of sources detected by the main X-ray observatories across the years. Horizontal lines represent mission lifetimes. Sco X-1 referes to the name of the first observed source. \fix{Add IXPE}}
    \label{fig:xsources}
\end{figure}
